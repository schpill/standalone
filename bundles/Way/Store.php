<?php
    ...
    class Store
    {
        ...
        {
            if (!is_dir(STORAGE_PATH . DS . 'way')) {
                File::mkdir(STORAGE_PATH . DS . 'way');
            }

            $this->dir = STORAGE_PATH . DS . 'way' . DS . $ns;
        ...
        {
            $key    = sha1($collection);
            $has    = Instance::has('WayStore', $key);

            if (true === $has) {
                return Instance::get('WayStore', $key);
            } else {
                return Instance::make('WayStore', $key, new self($collection));
            }
        }
