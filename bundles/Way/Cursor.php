<?php
    ...
    class Cursor implements Countable, Iterator
    {
        ...
        public function multisort($ids, $sorts)
        {
            ...
            return WayArray::fromArray($idsSorted);
        }

        public function toArray()
        {
            if (empty($this->selects)) {
                return WayArray::fromArray(array_map(function ($row) {
                    return $this->getRow($row);
                }, iterator_to_array($this->getIterator())));
            } else {
                $fields = $this->selects;

                return WayArray::fromArray(array_map(function ($id) use ($fields) {
                    $row = [];
                    $row['id'] = $id;
        ...
        public function cursor()
        {
            if (!isset($this->cursor)) {
                ...
                if (!$ids) {
                    ...
                    $ids = WayArray::fromArray($ids);
                } else {
                    if (is_object($ids)) {
                        $ids = WayArray::fromArray(iterator_to_array($ids));
                    } else {
                        $ids = WayArray::fromArray($ids);
                    }
                }
                ...
                if (isset($this->limit)) {
                    ...
                    $this->cursor = WayArray::fromArray(
                        array_slice(
