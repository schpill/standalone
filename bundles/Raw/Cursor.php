<?php
    ...
    class Cursor implements Countable, Iterator
    {
        ...
        public function multisort($ids, $sorts)
        {
            ...
            return RawArray::fromArray($idsSorted);
        }
        ...
        public function toArray()
        {
            ...
            if (empty($this->selects)) {
                return RawArray::fromArray(array_map(function ($row) {
                    $row = $this->getRow($row);
                ...
                return RawArray::fromArray(array_map(function ($id) use ($fields) {
                    $row = [];
                    $row['id'] = $id;
        ...
        public function cursor()
        {
            if (!isset($this->cursor)) {
                ...
                if (!$ids) {
                    ...
                    $ids = RawArray::fromArray($ids);
                } else {
                    if (is_object($ids)) {
                        $ids = RawArray::fromArray(iterator_to_array($ids));
                    } else {
                        $ids = RawArray::fromArray($ids);
                    }
                }
                ...
                if (isset($this->limit)) {
                    ...
                    $this->cursor = RawArray::fromArray(
                        array_slice(
                            $this->cursor,
                            $offset,
                            $this->limit
        ...
        private function compare($comp, $op, $value)
        {
            ...
            if (strlen($comp) && strlen($op) && strlen($value)) {
                ...
                switch ($op) {
                    ...
                    case '>=':
                        $res = floatval($comp) >= floatval($value);
                        break;
