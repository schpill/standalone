<?php
    ...
    class Db
    {
        ...
        public function __construct($db, $table)
        {
            ...
            lib('facade', ['SplFixedArray', 'MliteArray']);
        }
        ...
        public static function instance($db, $table)
        {
            ...
            $has    = Instance::has('Mlite', $key);

            if (true === $has) {
                return Instance::get('Mlite', $key);
            } else {
                return Instance::make('Mlite', $key, new self($db, $table));
            }
        }
        ...
        public function model($data = [])
        {
            ...
            $dir = Conf::get('dir.mlite.models', APPLICATION_PATH . DS . 'models' . DS . 'Mlite');
            ...
            if (!File::exists($modelFile)) {
                File::put($modelFile, str_replace('##class##', ucfirst(Inflector::lower($db)) . ucfirst(Inflector::lower($table)) . 'MliteModel', File::read(__DIR__ . DS . 'dbModel.tpl')));
            }

            $class = '\\Thin\\' . ucfirst(Inflector::lower($db)) . ucfirst(Inflector::lower($table)) . 'MliteModel';
        ...
        public function find($id, $object = true)
        {
            ...
            while($obj = $result->fetchArray(SQLITE3_ASSOC)) {
                break;
            }
        ...
        private function add(array $data, $checkTuple = true)
        {
            ...
            $query = "INSERT INTO $this->table (id, created_at, updated_at, ";
            ...
            $query = substr($query, 0, -2) . ") VALUES ('$id', '" . $data['created_at'] . "', '" . $data['updated_at'] . "', ";

            foreach ($data as $k => $v) {
                if ($k != 'id' && $k != 'created_at' && $k != 'updated_at') {
                    $query .= "'" . SQLite3::escapeString($v) . "', ";
                }
            }

            $query = substr($query, 0, -2) . ');';

            $result = $this->store->db->exec($query);
        ...
        private function edit($id, array $data, $checkTuple = true)
        {
            ...
            foreach ($data as $k => $v) {
                if ($k != 'id') {
                    $query .= "$k = '" . SQLite3::escapeString($v) . "', ";
                }
            }
        ...
        public function get($closure = null, $object = false)
        {
            ...
            if (!empty($this->wheres)) {
                ...
                foreach ($this->wheres as $wh) {
                    ...
                    if (!$first) {
                        $query .= " $op $f $o '" . SQLite3::escapeString($v) . "'";
                    } else {
                        $query .= " WHERE $f $o '" . SQLite3::escapeString($v) . "'";
                        $first = false;
                    }
                }
            }
        ...
        {
            $dir = Conf::get('dir.mlite.migrations', APPLICATION_PATH . DS . 'migrations' . DS . 'Mlite');
        ...
        {
            $dir = Conf::get('dir.mlite.seeds', APPLICATION_PATH . DS . 'seeds' . DS . 'Mlite');
