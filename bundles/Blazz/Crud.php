<?php
    ...
    class Crud
    {
        ...
        public function __construct(Database $model)
        {
            ...
            $fileConfig = APPLICATION_PATH . DS . 'models' . DS . 'CrudBlazz' . DS . ucfirst(Inflector::camelize($model->db)) . DS . ucfirst(Inflector::camelize($model->table)) . '.php';
        ...
        public static function instance(Database $model)
        {
            ...
            $has    = Instance::has('BlazzCrud', $key);

            if (true === $has) {
                return Instance::get('BlazzCrud', $key);
            } else {
                return Instance::make('BlazzCrud', $key, new self($model));
            }
        }
