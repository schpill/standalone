<?php
    ...
    class Crud
    {
        ...
        public function __construct(Database $model)
        {
            ...
            $fileConfig = APPLICATION_PATH . DS . 'models' . DS . 'CrudMy' . DS . ucfirst(Inflector::camelize($model->db)) . DS . ucfirst(Inflector::camelize($model->table)) . '.php';
        ...
        public static function instance(Database $model)
        {
            ...
            $has    = Instance::has('MyCrud', $key);

            if (true === $has) {
                return Instance::get('MyCrud', $key);
            } else {
                return Instance::make('MyCrud', $key, new self($model));
            }
        }
        ...
        public function listing($customFields = false)
        {
            ...
            $db = call_user_func_array(['\\My\\Db', 'instance'], [$this->model->db, $this->model->table]);
