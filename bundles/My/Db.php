<?php
    ...
    class Db
    {
        ...
        public function model($data = [])
        {
            ...
            $modelFile = APPLICATION_PATH . DS . 'models' . DS . 'My' . DS . 'models' . DS . Inflector::lower($db) . DS . ucfirst(Inflector::lower($table)) . '.php';

            if (!is_dir(APPLICATION_PATH . DS . 'models' . DS . 'My')) {
                File::mkdir(APPLICATION_PATH . DS . 'models' . DS . 'My');
            }

            if (!is_dir(APPLICATION_PATH . DS . 'models' . DS . 'My' . DS . 'models')) {
                File::mkdir(APPLICATION_PATH . DS . 'models' . DS . 'My' . DS . 'models');
            }

            if (!is_dir(APPLICATION_PATH . DS . 'models' . DS . 'My' . DS . 'models' . DS . Inflector::lower($db))) {
                File::mkdir(APPLICATION_PATH . DS . 'models' . DS . 'My' . DS . 'models' . DS . Inflector::lower($db));
            }

            if (!File::exists($modelFile)) {
                File::put($modelFile, str_replace('##class##', ucfirst(Inflector::lower($db)) . ucfirst(Inflector::lower($table)) . 'MyModel', File::read(__DIR__ . DS . 'dbModel.tpl')));
            }

            $class = '\\Thin\\' . ucfirst(Inflector::lower($db)) . ucfirst(Inflector::lower($table)) . 'MyModel';
