<?php
    ...
    class Db
    {
        public $db, $table, $collection, $results, $cnx, $limit, $offset, $groupBy, $cacheClient, $store;
        public $wheres          = [];
        public $selects         = [];
        public $orders          = [];
        public $groupBys        = [];
        ...
        public function __construct($db, $table)
        {
            ...
            lib('facade', ['SplFixedArray', 'LiveArray']);
        }
        ...
        public function model($data = [])
        {
            ...
            $modelFile = APPLICATION_PATH . DS . 'models' . DS . 'Live' . DS . 'models' . DS . Inflector::lower($db) . DS . ucfirst(Inflector::lower($table)) . '.php';

            if (!is_dir(APPLICATION_PATH . DS . 'models' . DS . 'Live')) {
                File::mkdir(APPLICATION_PATH . DS . 'models' . DS . 'Live');
            }

            if (!is_dir(APPLICATION_PATH . DS . 'models' . DS . 'Live' . DS . 'models')) {
                File::mkdir(APPLICATION_PATH . DS . 'models' . DS . 'Live' . DS . 'models');
            }

            if (!is_dir(APPLICATION_PATH . DS . 'models' . DS . 'Live' . DS . 'models' . DS . Inflector::lower($db))) {
                File::mkdir(APPLICATION_PATH . DS . 'models' . DS . 'Live' . DS . 'models' . DS . Inflector::lower($db));
            }

            if (!File::exists($modelFile)) {
                File::put($modelFile, str_replace('##class##', ucfirst(Inflector::lower($db)) . ucfirst(Inflector::lower($table)) . 'LiveModel', File::read(__DIR__ . DS . 'dbModel.tpl')));
            }

            $class = '\\Thin\\' . ucfirst(Inflector::lower($db)) . ucfirst(Inflector::lower($table)) . 'LiveModel';
        ...
        {
            return $object ? with(new Cursor($this))->toObject() : with(new Cursor($this))->toArray();
        }
