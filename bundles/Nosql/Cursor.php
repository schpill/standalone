<?php
    ...
    class Cursor implements Countable, Iterator
    {
        ...
        public function multisort($ids, $sorts)
        {
            ...
            return NosqlArray::fromArray($idsSorted);
        }
        ...
        public function toArray()
        {
            ...
            if (empty($this->selects)) {
                return NosqlArray::fromArray(array_map(function ($row) {
                    $row = $this->getRow($row);
                ...
                return NosqlArray::fromArray(array_map(function ($id) use ($fields) {
                    $row = [];
                    $row['id'] = $id;
        ...
        public function toObject()
        {
            ...
            if (empty($this->selects)) {
                return NosqlArray::fromArray(array_map(function ($row) {
                    $row = $this->db->model($this->getRow($row));
        ...
        public function cursor()
        {
            if (!isset($this->cursor)) {
                ...
                if (!$ids) {
                    ...
                    $ids = NosqlArray::fromArray($ids);
                } else {
                    if (is_object($ids)) {
                        $ids = NosqlArray::fromArray(iterator_to_array($ids));
                    } else {
                        $ids = NosqlArray::fromArray($ids);
                    }
                ...
                if (isset($this->limit)) {
                    ...
                    $this->cursor = is_array($this->cursor) ? $this->cursor : iterator_to_array($this->cursor);

                    $this->cursor = NosqlArray::fromArray(
                        array_slice(
                            $this->cursor,
                            $offset,
                            $this->limit
        ...
        {
            $cursor = $this->getIterator();
            $cursor = is_array($this->cursor) ? $this->cursor : iterator_to_array($this->cursor);
        ...
        private function makeFieldValues($field, $ids, $operator, $value)
        {
            ...
            if (!$values) {
                ...
                foreach ($ids as $id) {
                    $id = str_replace('ids.', '', $id);
                    $val = $this->store->hget('row.' . $id, $field, null);

                    if ($value == 'null' || is_null($value) || empty($value)) {
                        if ($operator == 'IS') {
                            $check = empty($val) || is_null($val);
                        } elseif ($operator == 'IS NOT' || $operator == 'ISNOT' ) {
                            $check = !empty($val) || !is_null($val);
                        } else {
                            if ($value == '0') {
