<?php
    ...
    class Util
    {
        ...
        {
            return self::normalizeDirname(dirname($path));
        }
        ...
        public static function emulateDirectories(array $listing)
        {
            ...
            foreach ($listing as $object) {
                list($directories, $listedDirectories) = self::emulateObjectDirectories(
                    $object,
                    $directories,
                    $listedDirectories
                );
            ...
            foreach ($directories as $directory) {
                $listing[] = self::pathinfo($directory) + ['type' => 'dir'];
            }
        ...
        {
            if (ftell($resource) !== 0 && self::isSeekableStream($resource)) {
                rewind($resource);
            }
        }
