<?php
    ...
    class FinediffLibInsertOp extends FinediffLibOp {
    ...
    class FinediffLibReplaceOp extends FinediffLibOp {
    ...
    class FinediffLibCopyOp extends FinediffLibOp {
        public function __construct($len) {
            $this->len = $len;
            }
        public function getFromLen() {
    ...
    * Collection of ops
    */
    class FinediffLibOps {
        public function appendOpcode($opcode, $from, $from_offset, $from_len) {
            if ( $opcode === 'c' ) {
                $edits[] = new FinediffLibCopyOp($from_len);
                }
            else if ( $opcode === 'd' ) {
                $edits[] = new FinediffLibDeleteOp($from_len);
                }
            else /* if ( $opcode === 'i' ) */ {
                $edits[] = new FinediffLibInsertOp(substr($from, $from_offset, $from_len));
                }
            }
        public $edits = array();
        }
    ...
    */
    class FinediffLib {
        ...
        * Constructor
        * ...
        * The $granularityStack allows FinediffLib to be configurable so that
        ...
        */
        public function __construct($from_text = '', $to_text = '', $granularityStack = null) {
            ...
            $this->granularityStack = $granularityStack ? $granularityStack : FinediffLib::$characterGranularity;
            $this->edits = array();
        ...
        public function renderDiffToHTML() {
            ...
            foreach ( $this->edits as $edit ) {
                $n = $edit->getFromLen();
                if ( $edit instanceof FinediffLibCopyOp ) {
                    FinediffLib::renderDiffToHTMLFromOpcode('c', $this->from_text, $in_offset, $n);
                    }
                else if ( $edit instanceof FinediffLibDeleteOp ) {
                    FinediffLib::renderDiffToHTMLFromOpcode('d', $this->from_text, $in_offset, $n);
                    }
                else if ( $edit instanceof FinediffLibInsertOp ) {
                    FinediffLib::renderDiffToHTMLFromOpcode('i', $edit->getText(), 0, $edit->getToLen());
                    }
                else /* if ( $edit instanceof FinediffLibReplaceOp ) */ {
                    FinediffLib::renderDiffToHTMLFromOpcode('d', $this->from_text, $in_offset, $n);
                    FinediffLib::renderDiffToHTMLFromOpcode('i', $edit->getText(), 0, $edit->getToLen());
                    }
                $in_offset += $n;
        ...
        */
        public static function getDiffOpcodes($from, $to, $granularities = null) {
            ...
            $diff = new FinediffLib($from, $to, $granularities);
        ...
        */
        public static function getDiffOpsFromOpcodes($opcodes) {
            $diffops = new FinediffLibOps();
            FinediffLib::renderFromOpcodes(null, $opcodes, array($diffops,'appendOpcode'));
        ...
        */
        public static function renderToTextFromOpcodes($from, $opcodes) {
            ob_start();
            FinediffLib::renderFromOpcodes($from, $opcodes, array('FinediffLib','renderToTextFromOpcode'));
            return ob_get_clean();
            }
        ...
        */
        public static function renderDiffToHTMLFromOpcodes($from, $opcodes) {
            $from = mb_convert_encoding($from, 'HTML-ENTITIES', 'UTF-8');
            ob_start();
            FinediffLib::renderFromOpcodes($from, $opcodes, array('FinediffLib','renderDiffToHTMLFromOpcode'));
            return ob_get_clean();
            }
        ...
        */
        ...
        public static $paragraphGranularity = array(
            FinediffLib::paragraphDelimiters
            );
        const sentenceDelimiters = ".\n\r";
        public static $sentenceGranularity = array(
            FinediffLib::paragraphDelimiters,
            FinediffLib::sentenceDelimiters
            );
        const wordDelimiters = " \t.\n\r";
        public static $wordGranularity = array(
            FinediffLib::paragraphDelimiters,
            FinediffLib::sentenceDelimiters,
            FinediffLib::wordDelimiters
            );
        ...
        public static $characterGranularity = array(
            ...
            FinediffLib::sentenceDelimiters,
            FinediffLib::wordDelimiters,
            FinediffLib::characterDelimiters
            );
        ...
        */
        private function _processGranularity($from_segment, $to_segment) {
            ...
            foreach ( FinediffLib::doFragmentDiff($from_segment, $to_segment, $delimiters) as $fragment_edit ) {
                // increase granularity
                if ( $fragment_edit instanceof FinediffLibReplaceOp && $has_next_stage ) {
                ...
                else if ( $fragment_edit instanceof FinediffLibCopyOp && $this->last_edit instanceof FinediffLibCopyOp ) {
                    $this->edits[count($this->edits)-1]->increase($fragment_edit->getFromLen());
                    $this->from_offset += $fragment_edit->getFromLen();
                    }
                else {
                    /* $fragment_edit instanceof FinediffLibCopyOp */
                    /* $fragment_edit instanceof FinediffLibDeleteOp */
                    /* $fragment_edit instanceof FinediffLibInsertOp */
        ...
        */
        private static function doFragmentDiff($from_text, $to_text, $delimiters) {
            ...
            // diffing.
            if ( empty($delimiters) ) {
                return FinediffLib::doCharDiff($from_text, $to_text);
                }
            ...
            $from_fragments = FinediffLib::extractFragments($from_text, $delimiters);
            $to_fragments = FinediffLib::extractFragments($to_text, $delimiters);
            ...
            while ( $job = array_pop($jobs) ) {
                ...
                if ( !$from_segment_length || !$to_segment_length ) {
                    if ( $from_segment_length ) {
                        $result[$from_segment_start * 4] = new FinediffLibDeleteOp($from_segment_length);
                        }
                    else if ( $to_segment_length ) {
                        $result[$from_segment_start * 4 + 1] = new FinediffLibInsertOp(substr($to_text, $to_segment_start, $to_segment_length));
                        }
                    continue;
                    }
                ...
                if ( $best_copy_length ) {
                    ...
                    $result[$best_from_start * 4 + 2] = new FinediffLibCopyOp($best_copy_length);
                    $jobs[] = array($best_from_start + $best_copy_length, $from_segment_end, $best_to_start + $best_copy_length, $to_segment_end);
                    }
                else {
                    $result[$from_segment_start * 4 ] = new FinediffLibReplaceOp($from_segment_length, substr($to_text, $to_segment_start, $to_segment_length));
                    }
                }
        ...
        private static function doCharDiff($from_text, $to_text) {
            ...
            while ( $job = array_pop($jobs) ) {
                ...
                if ( !$from_segment_len || !$to_segment_len ) {
                    if ( $from_segment_len ) {
                        $result[$from_segment_start * 4 + 0] = new FinediffLibDeleteOp($from_segment_len);
                        }
                    else if ( $to_segment_len ) {
                        $result[$from_segment_start * 4 + 1] = new FinediffLibInsertOp(substr($to_text, $to_segment_start, $to_segment_len));
                        }
                    continue;
                    }
                if ( $from_segment_len >= $to_segment_len ) {
                ...
                if ( $copy_len ) {
                    ...
                    $result[$from_copy_start * 4 + 2] = new FinediffLibCopyOp($copy_len);
                    $jobs[] = array($from_copy_start + $copy_len, $from_segment_end, $to_copy_start + $copy_len, $to_segment_end);
                    }
                // no match,  so delete all, insert all
                else {
                    $result[$from_segment_start * 4] = new FinediffLibReplaceOp($from_segment_len, substr($to_text, $to_segment_start, $to_segment_len));
                    }
                }
            ksort($result, SORT_NUMERIC);
            return array_values($result);
