<?php
    ...
    class ShortcodeLib
    {
        public function isBot($ua = null)
        {
            if (is_null($ua) && isset($_SERVER['HTTP_USER_AGENT'])) {
        ...
        public function lastAsset($file, $min = false)
        {
            $fileD = $min ? Config::get('app.module.dir') . DS . $file : Config::get('app.module.dir') . $file;

            if (File::exists($fileD)) {
                $age = File::age($fileD);

                return $file . '?' . sha1($age);
            }
