<?php
    ...
    class PaypalLib
    {
        ...
        {
            return defined('URLSITE') ? URLSITE : '/';
        }
         ...
         * @return Paypal\Api\Payment
         */
        public function getById($paymentId, $apiContext = null)
        {
            if (isset($apiContext)) {
         ...
         * @return Paypal\Api\Payment
         */
        public function getAll($param, $apiContext = null)
        {
            if (isset($apiContext)) {
        ...
        {
            $context = $this->ApiContext(Config::get('application.paypal.id'), Config::get('application.paypal.id'));

            $context->setConfig([
                'mode'                      => APPLICATION_ENV != 'production' ? 'sandbox' : 'live',
                'service.EndPoint'          => APPLICATION_ENV != 'production' ? 'https://api.sandbox.paypal.com' : 'https://api.paypal.com',
                'http.ConnectionTimeOut'    => 30,
                'log.LogEnabled'            => true,
                'log.FileName'              => LOGS_PATH . DS . 'PayPal.log',
                'log.LogLevel'              => 'FINE'
            ]);
