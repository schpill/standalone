<?php
    ...
    class GexfLib
    {
        ...
        private $creator = "tools.clippcity.com";
        private $mode = "static";
        ...
        {
            if (!$this->nodeExists($node)) $this->nodeObjects[$node->id] = $node;
        ...
        {
            if (!$this->childExists($child, $parent)) $this->nodeObjects[$parent->id]->addNodeChild($child);
        ...
        public function addEdge($source, $target, $weight = 1)
        {
            ...
            if (array_key_exists($edge->id, $this->edgeObjects) == false) $this->edgeObjects[$edge->id] = $edge;
            // else add weight to existing edge
            else $this->edgeObjects[$edge->id]->addToEdgeWeight($weight);
        ...
        {
            if (array_key_exists($eid, $this->edgeObjects) == false) die('make an edge before you add a spell');
        ...
        public function renderNodes($nodeObjects)
        {
            ...
            foreach ($nodeObjects as $id => $node) {
                ...
                // add position
                if ($node->position != array()) {
                    $xmlNodes .= '<viz:position x="' . $node->position['x'] . '" y="' . $node->position['y'] . '" z="' . $node->position['z'] . '"/>';
                }

                // add size
                if ($node->size) {
                    $xmlNodes .= '<viz:size value="' . $node->size . '"/>';
                }
                ...
                if (!empty($node->spells)) {
                    $xmlNodes .= "<spells>\n";

                    foreach ($node->spells as $spell) {
                        $xmlNodes .= '<spell' .
                        (isset($spell->startdate) ? ' start="' . $spell->startdate . '"' : '') .
                        (isset($spell->enddate) ? ' end="' . $spell->enddate . '"' : '') . " />\n";
                    }
        ...
        public function renderNodeAttributes()
        {
            ...
            if (!empty($this->nodeAttributeObjects)) {
        ...
        public function renderEdgeAttributes()
        {
            ...
            if (!empty($this->edgeAttributeObjects)) {
    ...
    class GexfNode
    {
        ...
        {
            if (isset($idprefix)) $this->id = $idprefix . md5($this->name);
            else $this->id = "n-" . md5($this->name);
        }
        ...
        public function getNodeAttributeValue($attributeName)
        {
            ...
            if (isset($this->attributes[$aid])) return $this->attributes[$aid]->getAttributeValue();
            else return false;
        }
        ...
        public function setNodePosition($x, $y, $z = '0.0')
        {
            $this->position = array("x" => $x, "y" => $y, "z" => floatval($z));
        }
        ...
        public function setNodeSize($size)
        {
    ...
    class GexfEdge
    {
        ...
        public function setEdgeId()
        {
            ...
            if ($this->edgeType == "undirected") sort($sort);
    ...
    class GexfSpell
    {
        ...
        {
            if (!preg_match("/\d{4}-\d{2}-\d{2}/", $date)) throw Exception("Time not in right format");
