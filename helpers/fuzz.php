<?php
    ...
    class FuzzLib
    {
        private $query = [], $store, $db, $table, $res, $resource, $write = false;
        ...
        public function age()
        {
            return $this->store->age($this->db . '.' . $this->table);
        }

        public function __destruct()
        {
            if (true === $this->write) {
                $this->store->set($this->db . '.' . $this->table, $this->collection());
            }
        }
        ...
        {
            return fmr('fuzz')->aged($key, function () use ($data) {
                $merged = $ids = [];
