<?php
    ...
    class ConvertLib
    {
         ...
         * @return array|string
         */
        public function raw2att($val)
        {
            return $this->raw2xml($val);
        }
         ...
         * @return array|string
         */
        public function raw2htmlatt($val)
        {
            return $this->raw2att($val);
        }
         ...
         * @return array|string
         */
        public function raw2htmlname($val)
        {
            if (is_array($val)) {
                foreach ($val as $k => $v) {
                    $val[$k] = $this->raw2htmlname($v);
                }
         ...
         * @return array|string
         */
        public function raw2htmlid($val)
        {
            if (is_array($val)) {
                foreach ($val as $k => $v) {
                    $val[$k] = $this->raw2htmlid($v);
                }
         ...
         * @return array|string
         */
        public function raw2xml($val)
        {
            if (is_array($val)) {
                foreach ($val as $k => $v) $val[$k] = $this->raw2xml($v);
                return $val;
            } else {
         ...
         * @return array|string
         */
        public function raw2js($val)
        {
            if (is_array($val)) {
                foreach ($val as $k => $v) $val[$k] = $this->raw2js($v);
                return $val;
            } else {
                return str_replace(
         ...
         * @return string JSON encoded string
         */
        public function array2json($val)
        {
            return $this->raw2json($val);
        }
         ...
         */
        public function xml2raw($val)
        {
            if (is_array($val)) {
                foreach ($val as $k => $v) $val[$k] = $this->xml2raw($v);
                return $val;
            } else {
                // More complex text needs to use html2raw instead
                if (strpos($val,'<') !== false) return $this->html2raw($val);
         ...
         */
        public function json2array($val)
        {
         ...
         * @return array
         */
        public function xml2array($val, $disableDoctypes = false, $disableExternals = false)
        {
            // Check doctype
        ...
        protected function recursiveXMLToArray($xml)
        {
            ...
            if (is_array($xml)) {
                ...
                foreach ($xml as $key => $value) {
                    $r[$key] = $this->recursiveXMLToArray($value);
                }
         ...
         */
        public function linkIfMatch($string)
        {
            if (preg_match( '/^[a-z+]+\:\/\/[a-zA-Z0-9$-_.+?&=!*\'()%]+$/', $string )) return "<a style=\"white-space: nowrap\" href=\"$string\">$string</a>";
            else return $string;
        }
         ...
         * @return string
         */
        public function html2raw($data, $preserveLinks = false, $wordWrap = 0, $config = null)
        {
            $defaultConfig = array(
                'PreserveLinks' => false,
         ...
         */
        public function raw2mailto($data)
        {
            return str_ireplace(
                array("\n",'?','=',' ','(',')','&','@','"','\'',';'),
         ...
         * @return string
         */
        public function nl2os($data, $nl = PHP_EOL)
        {
         ...
         * @return string
         */
        public function base64url_encode($val)
        {
            return rtrim(
                strtr(
                    base64_encode(
         ...
         */
        public function base64url_decode($val)
        {
            return json_decode(
                base64_decode(
                    str_pad(
